...
export const createRequest = async (req, res) => {
    try {
        // Use req.staffId from requireAuth middleware instead of req.body.staffAuthId
...
export const updateRequest = async (req, res) => {
    try {
        // Calls RequestService.updateRequest
...
export const forwardRequest = async (req, res) => {
    try {
        const { id } = req.params;
        const { recipientStaffId, note } = req.body;
        if (!recipientStaffId) {
            return res.status(400).json({ error: 'recipientStaffId is required' });
        }
        const result = await RequestService.forwardRequest(id, {
            recipientStaffId,
            note,
            actorStaffId: req.staffId, // Set by requireAuth middleware
        });
        res.status(200).json(result);
    } catch (error) { 
        res.status(500).json({ error: error.message }); 
