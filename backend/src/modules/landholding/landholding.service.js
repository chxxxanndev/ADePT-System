...
class LandholdingService {
    ...
    async updateDraft(id, formData) {
        // 1. Update the main certificate record
        const { data: cert, error: certErr } = await supabase
            .from('encoded_landholding_certificates')
            .update({
        ...
        // 2. Update each property row individually (only rows that have their own id)
        const properties = formData.properties || [];
        for (const row of properties) {
            if (!row.id) continue; // skip rows without an id (shouldn't normally happen on edit)
        ...
        // 3. Re-fetch the full hydrated record so the frontend gets updated properties back
        ...
        return { ...cert, properties: refreshedProps };
    }
}
