...
class AuthService {
  ...
  async loginUser({ username, password }) {
    ...
    if (!username.includes('@')) {
      // NOTE: this now calls a SECURITY DEFINER Postgres function instead of
      // querying the `staff` table directly. A direct .from('staff').select(...)
      // here fails under RLS, because at this point in the flow the user is NOT
      // authenticated yet (auth.uid() is null), so the "Admins can view staff"
      // policy can never pass — it always returned null and threw "Username not
      // found." even for valid usernames. The RPC function runs with elevated
      // privileges and exposes only the one field needed for login.
