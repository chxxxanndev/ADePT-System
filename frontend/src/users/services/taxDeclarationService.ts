...
export const taxDeclarationService = {
     ...
     */
    // Inside taxDeclarationService.ts
...
getTaxDeclaration: async (requestId: string) => {
    try {
        ...
        // 1. Process the rows
        const processedRows = (dbData.encoded_assessment_rows || []).map((row: any) => ({
            id: row.id, // Ensure ID is passed for the updateDraft logic
        ...
        const totalArea = processedRows.reduce((sum: number, r: any) => sum + (parseFloat(r.area) || 0), 0);
        const areaUnitSuffix = processedRows[0]?.areaUnit || '';
        ...
        return {
            ...
            // CHANGE THIS KEY NAME HERE:
            assessments: processedRows, 
        };
    } catch (error) {
