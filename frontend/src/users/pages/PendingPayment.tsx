        ...
        <div className="pp-page">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', background: '#fff', padding: '10px 16px', borderRadius: '10px', border: '1px solid #ddd', width: '320px' }}>
                    <SearchIcon />
                    <input placeholder="Search..." onChange={(e) => setSearchQuery(e.target.value)} style={{ border: 'none', outline: 'none', width: '100%', marginLeft: '10px' }} />
                </div>
            </div>
                        ...
                        {loading ? <tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px' }}>Loading...</td></tr> :
                            filtered.map((p) => (
                                <tr key={p.id} onClick={() => onSelectPayment(p)} style={{ borderBottom: '1px solid #eee', cursor: 'pointer' }}>
                                    <td style={{ padding: '16px 20px', fontWeight: 700, fontFamily: 'monospace' }}>{p.referenceNumber}</td>
                                    <td style={{ padding: '16px 20px' }}>{p.declarant_name}</td>
                                    <td style={{ padding: '16px 20px' }}><span style={{ background: '#f1f5f9', padding: '4px 8px', borderRadius: '4px' }}>{p.documentType}</span></td>
                                    <td style={{ padding: '16px 20px', fontWeight: 800, color: '#059669' }}>₱{Number(p.amountDue).toFixed(2)}</td>
                                    <td style={{ padding: '16px 20px' }}>{p.dateRequested}</td>
                                    <td style={{ padding: '16px 20px', textAlign: 'center' }}>
                                        <button onClick={(e) => handleArchive(e, p.referenceNumber, p.id)} style={{ background: '#fee2e2', border: 'none', color: '#ef4444', padding: '8px 10px', borderRadius: '6px', cursor: 'pointer' }}><ArchiveIcon /></button>
                                    </td>
                                </tr>
                            ))
                        }
